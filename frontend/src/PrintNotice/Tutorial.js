...
function Tutorial() {
  ...
  const [courseLabChiefDetails, setCourseLabChiefDetails] = useState({})
  ...
  const handleSelectExamResponse = (e) => {
    ...
    axios
      .get(`http://127.0.0.1:8000/core/exam-responsibility-detail/${selectedResponsibilityId}/`)
      .then((response) => {
        setSelectedExamResponsibilityDetails(response.data);
  
        const labCourseInfo = response.data.course_lab_tutorial.lab_course_info;
        axios
          .get(`http://127.0.0.1:8000/core/lab-course-chief-detail/`)
          .then((chiefResponse) => {
            setCourseLabChiefDetails(chiefResponse.data);
          })
          .catch((error) => {
            console.error('Error fetching lab course chief details:', error);
          });
      })
      .catch((error) => {
        console.error('Error fetching exam responsibility details:', error);
      });
  };
...
{selectedExamResponsibilityDetails.lab_exam_invigilation_schedule.id}
</p>
    ...
    {courseLabChiefDetails && (
            <table>
              <tbody>
                ...
                <td>
                  {courseLabChiefDetails.course_lab_tutorial.lab_course_info.course_code}<br />
                  {courseLabChiefDetails.lab_unit}<br />
                  {courseLabChiefDetails.faculty}<br />
                  {courseLabChiefDetails.student_count}<br />
                </td>
              </tbody>
            </table>
          )}
