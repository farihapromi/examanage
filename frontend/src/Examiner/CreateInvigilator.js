...
useEffect(() => {
    ...
    axios
      .get('http://127.0.0.1:8000/core/course-schedule-detail/')
      .then((response) => {
...
const handleCourseScheduleChange = (e) => {
  ...
  const handleSubmit = () => {
    ...
    const data = {
      ...
      course_schedule:selectedCourseSchedule,
      invigilator:selectedInvigilator,
      
      
    };
    console.log(data)
    axios.post("http://127.0.0.1:8000/core/invigilator/", data).then((response) => {
  ...
  return (
    <div>
        <h1>Create Theory  Invigilator Schedule</h1>
<form action="">
...
<div>
      ...
      {/* for invigilator */}
      <div>
        <label htmlFor="inivigilator" className='class-label'>Select invigilator</label>&nbsp;
...
export default CreateInvigilator